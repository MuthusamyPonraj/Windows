...
# Get the Theta value from Negative Binomial formula
negBin<- glm.nb(price~1, data=trainData ,maxit=1000, trace=T,link=sqrt)

# Applying General Regression Model - Sqrt function to predict price
...
imports_GLM = glm(importsFormula , trainData, family=negative.binomial(negBin$theta, link="sqrt"))
summary(imports_GLM)
