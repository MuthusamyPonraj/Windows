...
# Frame the data. 
WinNF = as.character(as.integer(glass$type == 'WinNF'))
glass = cbind(glass[, 1:9], WinNF)

# Randomizing data
glass<- glass[sample(nrow(glass)),]

# Divide dataset for training and test
trainData<-glass[1:170,]
testData<-glass[171:214,]

# Applying General Regression Model - cloglog function to predict WinNF type
glassFormula = formula(WinNF~ .)
glass_GLM = glm(glassFormula , trainData, family = binomial(link="logit"))
summary(glass_GLM )

# Display the predicted results and create cross table to check on accuracy
# Predict "WinNF" column probability for test data set
...
CrossTable(glassTestPrediction,testData$WinNF,  prop.chisq = FALSE, prop.t = FALSE, prop.r = FALSE,dnn = c('predicted', 'actual'))
...
# The first vector must contain the class values - WinNF column and
# The second vector must contain the estimated probability of the positive class(WindowNon-floatGlassTypeProbability)
pred <- prediction(labels = testData$WinNF, predictions = glassTestProbabilities)
...
confusionMatrix(glassTestPrediction,testData$WinNF, positive = "1")
...
# Frame the data. 
WinNF = as.character(as.integer(glassOriginal$type == 'WinNF'))
glassOriginal = cbind(glassOriginal[, 1:9], WinNF)
...
names(result) = c("Predicted_WindowNonFloatGlassType", "NonWindowNon-floatGlassTypeProbability", "WindowNon-floatGlassTypeProbability")
