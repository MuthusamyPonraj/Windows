...
namespace TitanicBinomialProbit
{
    /// <summary>
    ...
    public class Program
    {
        ...
        public object GetDataObject(Table inputTable, int row)
        {
            var titanic = new
            {
                pclass = inputTable[row, "pclass"],
                survived = inputTable[row, "survived"],
                sex = inputTable[row, "sex"],
                age = inputTable[row, "age"],
                sibsp = inputTable[row, "sibsp"],
                parch = inputTable[row, "parch"]
            };
        ...
        public bool Compare(Table table, int rColumnIndex, int predictedColumnIndex, ref string differentIndices)
        {
            ...
            for (int i = 0; i < table.RowCount; i++)
            {
                ...
                if (table[i, rColumnIndex].ToString() != outputTable[i, predictedColumnIndex].ToString())
                {
                    differentIndices += ", " + i;
                    isDifferent = true;
                }
