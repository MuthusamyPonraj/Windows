...
namespace OzoneQuasipoissonSqrt
{
    /// <summary>
    ...
    public class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            //Load input csv
            program.inputTable = new Table("../../Model/Ozone.csv", true, ',');
        ...
        public Table PredictResult(Table inputTable, string pmmlPath)
        {
            ...
            {
                var ozone = GetDataObject(inputTable, i);
        ...
        private void InitializeTable(int rowCount, string[] predictedCategories)
        {
            ...
            outputTable.ColumnNames[0] = "Predicted_AverageOzoneReading";
        }
        ...
        public object GetDataObject(Table inputTable, int row)
        {
            var ozone = new
            {
                Month = inputTable[row, "Month"],
                Day_of_month = inputTable[row, "Day_of_month"],
                Day_of_week = inputTable[row, "Day_of_week"],
                average_ozone_reading = inputTable[row, "average_ozone_reading"],
                pressure_height = inputTable[row, "pressure_height"],
                Wind_speed = inputTable[row, "Wind_speed"],
                Humidity = inputTable[row, "Humidity"],
                Temperature_at_Sandburg = inputTable[row, "Temperature_at_Sandburg"],
                Temperature_at_El_Monte = inputTable[row, "Temperature_at_El_Monte"],
                Inversion_base_height = inputTable[row, "Inversion_base_height"],
                Pressure_gradient = inputTable[row, "Pressure_gradient"],
                Inversion_base_temperature = inputTable[row, "Inversion_base_temperature"],
                Visibility = inputTable[row, "Visibility"]
            };
            return ozone;
        }
