...
namespace WineLog
{
    ...
    public class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            Console.WriteLine("\nResult saved in : " + Path.GetFullPath("../../Model/PredictedOutput.html"));
            Console.ReadKey();
        }
        ...
        public void ComparePredictedResultsWithR(string rOutputDataCSVPath)
        {
            ...
            bool isDifferent = Compare(rResults, 0, 0, ref differentIndices);
#if CONSOLE
        ...
        public object GetDataObject(Table inputTable, int row)
        {
            var wine = new
            {
                Type = inputTable[row, "Type"],
                Alcohol = inputTable[row, "Alcohol"],
                Malic = inputTable[row, "Malic"],
                Ash = inputTable[row, "Ash"],
                Alcalinity = inputTable[row, "Alcalinity"],
                Magnesium = inputTable[row, "Magnesium"],
                Phenols = inputTable[row, "Phenols"],
                Flavanoids = inputTable[row, "Flavanoids"],
                Nonflavanoids = inputTable[row, "Nonflavanoids"],
                Proanthocyanins = inputTable[row, "Proanthocyanins"],
                Color = inputTable[row, "Color"],
                Hue = inputTable[row, "Hue"],
                Dilution = inputTable[row, "Dilution"],
                Proline = inputTable[row, "Proline"]
            };
