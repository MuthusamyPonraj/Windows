...
# Applying General Regression Model - probit function to predict tip
coefficient<-glm(tip~1,data=trainData)
tipsFormula= formula(tip~total_bill+sex+smoker+day+time+size)
tips_GLM = glm(tipsFormula, trainData, family = quasipoisson(link="probit"),
start=c(coef(coefficient),0,0,0,0,0,0,0,0))
