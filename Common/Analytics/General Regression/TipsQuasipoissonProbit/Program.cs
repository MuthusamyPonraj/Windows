...
namespace TipsQuasipoissonProbit
{
    /// <summary>
    ...
    public class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            //Load input csv
            program.inputTable = new Table("../../Model/Tips.csv", true, ',');
        ...
        public Table PredictResult(Table inputTable, string pmmlPath)
        {
            ...
            {
                var tips = GetDataObject(inputTable, i);
        ...
        public object GetDataObject(Table inputTable, int row)
        {
            var tips = new
            {
                total_bill = inputTable[row, "total_bill"],
                tip = inputTable[row, "tip"],
                sex = inputTable[row, "sex"],
                smoker = inputTable[row, "smoker"],
                day = inputTable[row, "day"],
                time = inputTable[row, "time"],
                size = inputTable[row, "size"]
            };
            return tips;
        }
