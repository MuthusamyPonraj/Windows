...
# Applying the General Regression Model - inverse function to predict price
carsFormula = formula(Price~Mileage+Cylinder+Doors+Cruise+Sound+Leather+Buick+Cadillac+Chevy+Pontiac+Saab+convertible+hatchback+wagon)
cars_GLM = glm(carsFormula, trainData, family = inverse.gaussian(link="inverse"))
summary(cars_GLM)

# Display the predicted results
# Predict "Price" column probability for test data set
carsTestProbabilities = predict(cars_GLM,type = "response",testData)
# Display predicted probabilities
carsTestProbabilities

# PMML generation
pmmlFile<-pmml(cars_GLM,data=trainData)
...
carsEntireProbabilities = predict(cars_GLM, type = "response",carsOriginal)
