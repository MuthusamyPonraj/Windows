...
# Applying General Regression Model - logit function to predict Survived
titanicFormula = formula(Survived~sex+age+sibsp+parch)
titanic_GLM = glm(titanicFormula ,trainData , family = binomial(link="logit"))
summary(titanic_GLM)
...
# The second vector must contain the estimated probability of the positive class(SurvivedProbabilit)
...
names(result) = c("Predicted_Survived", "NotSurvivedProbability", "SurvivedProbabilit")
