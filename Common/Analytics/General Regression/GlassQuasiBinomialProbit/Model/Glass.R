...
# Frame the data. 
WinF = as.character(as.integer(glass$type == 'WinF'))
glass = cbind(glass[, 1:9], WinF)
...
# Applying General Regression Model - cloglog function to predict WinF type
glassFormula = formula(WinF~ .)
glass_GLM = glm(glassFormula, trainData, family = quasibinomial(link="probit"))
summary(glass_GLM )
