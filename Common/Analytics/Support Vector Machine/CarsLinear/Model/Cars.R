...
cars_SVM<-ksvm(Price~.,data=trainData,kernel="vanilladot")
cars_SVM

# Predict "Price" column for test data set
carsTestPrediction<-predict(cars_SVM,testData)
# Display predicted values
carsTestPrediction


# PMML generation
pmmlFile<-pmml(cars_SVM,data=trainData)
...
carsEntirePrediction<-predict(cars_SVM,carsOriginal)
