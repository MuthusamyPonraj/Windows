...
audit$Adjusted <- factor(audit$Adjusted, ordered = FALSE)
...
auditTestProbabilities = predict(auditTrainingModel, newdata = testData, threshold = 0, type = "raw")
...
# PMML generation 
saveXML(pmml(auditTrainingModel, predictedField = "Adjusted"), "Audit.pmml")
...
# Considering integer variable as factor
auditOriginal$Adjusted <- factor(auditOriginal$Adjusted, ordered = FALSE)

# Applying Naive Bayes model to entire dataset and save the results in a CSV file
classes = predict(auditTrainingModel, newdata = auditOriginal, threshold = 0, type = "class")
probabilities = predict(auditTrainingModel, newdata = auditOriginal, threshold = 0, type = "raw")
