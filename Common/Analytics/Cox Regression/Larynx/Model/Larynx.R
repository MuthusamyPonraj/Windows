...
trainData=larynx[1:72,]
testData=larynx[73:90,]

# Applying Cox Regression Model to predict Survival
larynx_Cox = coxph(Surv(time,delta)~stage+age+diagyr, trainData)
summary(larynx_Cox)
...
# Load Original "larynx" dataset from "KMsurv" package
data(larynx)

# Applying Cox Regression Model to predict Survival
larynx_Cox_Entire = coxph(Surv(time,delta)~stage+age+diagyr, trainData)
summary(larynx_Cox_Entire)

# Applying Cox Regression model to entire dataset and save the results in a CSV file
larynxEntirePrediction = predict(larynx_Cox_Entire, type = "expected", larynx)

# PMML generation
pmmlFile<-pmml(larynx_Cox_Entire, data=trainData)
