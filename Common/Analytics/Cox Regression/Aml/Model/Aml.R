...
aml_Cox = coxph(Surv(time,status)~x, aml)
summary(aml_Cox)
...
amlTestPrediction = predict(aml_Cox, type = "expected",aml)
...
# Load Original "aml" dataset from "survival" package
aml

# Applying Cox Regression model to entire dataset and save the results in a CSV file
amlEntirePrediction = predict(aml_Cox, type = "expected", aml)
