...
titanicTestProbabilities<-predict(titanic_NN,testData)
...
pred <- prediction(labels = testData$survived, predictions = titanicTestProbabilities[,1])
...
# PMML generation
pmmlFile<-pmml(titanic_NN,data=trainData)
...
titanicEntirePrediction<-predict(titanic_NN,titanicOriginal,type="class")

# Predict probability value for entire dataset
Predicted_probabilities<-predict(titanic_NN,titanicOriginal)

# Save predicted value and probabilities in a data frame
result = data.frame(titanicEntirePrediction,1-Predicted_probabilities,Predicted_probabilities)
