...
cars_NN<-nnet(Price~.,data=trainData ,size= 15 ,decay = 1e-3, maxit = 1000, linout = TRUE)
cars_NN

# Predict "Price" column for test data set
carsTestPrediction<-predict(cars_NN,testData)
# Display predicted values
carsTestPrediction

# PMML generation
pmmlFile<-pmml(cars_NN,data=trainData)
...
carsEntirePrediction<-predict(cars_NN,carsOriginal)
