...
breastCancer_NN<-nnet(cens~.,data=trainData,size=15)
breastCancer_NN

# Displays the predicted results and create cross table for accuracy
# Predict "Cens" column for test data set
breastCancerTestPrediction<-predict(breastCancer_NN,testData,type="class")
...
breastCancerTestProbabilities<-predict(breastCancer_NN,testData)
...
pred <- prediction(labels = testData$cens, predictions = breastCancerTestProbabilities[,1])
...
# PMML generation
pmmlFile<-pmml(breastCancer_NN,data=trainData)
...
breastCancerEntirePrediction<-predict(breastCancer_NN,breastCancerOriginal,type="class")

# Predict probability value for entire dataset
Predicted_probabilities<-predict(breastCancer_NN,breastCancerOriginal)

# Save predicted value and probabilities in a data frame
result = data.frame(breastCancerEntirePrediction,1-Predicted_probabilities,Predicted_probabilities)
