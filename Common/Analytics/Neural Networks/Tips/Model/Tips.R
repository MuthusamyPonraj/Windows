...
tips_NN<-nnet(tip~.,data=trainData,size=15,decay = 1e-3, maxit = 1000, linout = TRUE)
tips_NN

# Predict "Tip" column for test data set
tipsTestPrediction<-predict(tips_NN,testData)
# Display predicted values
tipsTestPrediction


# PMML generation
pmmlFile<-pmml(tips_NN,data=trainData)
...
tipsEntirePrediction<-predict(tips_NN,tipsOriginal)
