...
# Here we directly load the Imports dataset installed with the "randomForest" package.
data(imports85)
...
imports_RF<-randomForest(price~.,data=trainData,ntree=50)
imports_RF

# Predict "price" column for test data set
importsTestPrediction<-predict(imports_RF,testData)
# Display predicted values
importsTestPrediction


# PMML generation
pmmlFile<-pmml(imports_RF,data=trainData)
...
importsEntirePrediction<-predict(imports_RF,importsOriginal)
