...
tips_RF<-randomForest(tip~.,data=trainData,ntree=50)
tips_RF

# Predict "tip" column for test data set
tipsTestPrediction<-predict(tips_RF,testData)
# Display predicted values
tipsTestPrediction


# PMML generation
pmmlFile<-pmml(tips_RF,data=trainData)
...
tipsEntirePrediction<-predict(tips_RF,tipsOriginal)
