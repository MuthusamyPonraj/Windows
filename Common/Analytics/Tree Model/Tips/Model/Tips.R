...
tipsTrain <- tips[1:196, ]
tipsTest <- tips[197:244, ]

# Applying TreeModel
tipsModel <- rpart( tip ~., data = tipsTrain)
tipsModel

# Display the predicted results
# Predict "tip" column for test data set
tipsTestPrediction<- predict(tipsModel ,tipsTest)
# Display predicted values
tipsTestPrediction


# PMML generation
tipsPmml<- pmml(tipsModel, data = tipsTrain)
write (toString (tipsPmml), file="Tips.pmml")
saveXML (tipsPmml, file="Tips.pmml")
...
tipsEntirePrediction<- predict(tipsModel ,tipsOriginal)
