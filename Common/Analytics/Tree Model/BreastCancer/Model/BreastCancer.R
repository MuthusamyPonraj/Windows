...
cancer$tgrade<-factor(cancer$tgrade, ordered = FALSE)
...
cancer[ , "cens"] = as.factor(cancer[ , "cens"])
...
# Applying TreeModel
cancerModel <- rpart(cens ~ ., data = cancerTrain)
cancerModel
...
CrossTable(cancerTest$cens, cancerTestPrediction, prop.chisq=FALSE,prop.c=FALSE,prop.r=FALSE,dnn = c('actual values','predicted values'))     
...
pred <- prediction(labels = cancerTest$cens, predictions = cancerTestProbabilities[,2])
...
confusionMatrix(cancerTestPrediction,cancerTest$cens, positive = "1")
...
data(GBSG2)
# Omit rows with missing values
cancerOriginal = na.omit(GBSG2)

# Converting Ordered factor to normal factor
cancerOriginal$tgrade<-factor(cancerOriginal$tgrade, ordered = FALSE)

# Considering integer variable as factor
cancerOriginal[ , "cens"] = as.factor(cancerOriginal[ , "cens"])

# Apply model to entire dataset and write the results in a csv file
classes = predict(cancerModel, newdata = cancerOriginal, threshold = 0, type = "class")
probabilities = predict(cancerModel, newdata = cancerOriginal, threshold = 0, type = "prob")
