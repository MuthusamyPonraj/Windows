...
namespace Bfeed_Coxph
{
    ...
    public class Program
    {
        ...
        private void InitializeTable(int rowCount, string predictedfield, string[] predictedCategories)
        {
            ...
            outputTable.ColumnNames[0] = "Predicted_Survival";
        }
        ...
        public object GetDataObject(Table inputTable, int row)
        {
            var bfeed = new
            {
                duration = inputTable[row, "duration"],
                delta = inputTable[row, "delta"],
                race = inputTable[row, "race"],
                poverty = inputTable[row, "poverty"],
                smoke = inputTable[row, "smoke"],
                alcohol = inputTable[row, "alcohol"],
                agemth = inputTable[row, "agemth"],
                ybirth = inputTable[row, "ybirth"],
                yschool = inputTable[row, "yschool"],
                pc3mth = inputTable[row, "pc3mth"],
            };
            return bfeed;
        }
