...
#Use 'r2pmml' package version v0.3.0 or below since, the function 'quantile' has been deprecated in later versions. 
...
imports_GBM =  gbm(price~ symboling+normalizedLosses+fuelType+aspiration+
        numOfDoors+bodyStyle+driveWheels+wheelBase+length+width+height+
        curbWeight+engineSize+bore+stroke+compressionRatio+horsepower+
        peakRpm+cityMpg+highwayMpg,data=trainData, distribution=list(name="quantile",alpha=0.25))

# Display the predicted results 
# Predict "price" column probability for test data set
importsTestProbabilities = predict.gbm(imports_GBM, newdata=testData, n.trees=100,type="response")
...
importsEntireProbabilities = predict.gbm(imports_GBM, importsOriginal, n.trees=100,type="response")
