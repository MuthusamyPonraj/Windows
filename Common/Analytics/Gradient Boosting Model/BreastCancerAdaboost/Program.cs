...
namespace BreastCancer_Adaboost
{
    /// <summary>
    ...
    public class Program
    {
        ...
        private void InitializeTable(int rowCount, string[] predictedCategories)
        {
            ...
            outputTable.ColumnNames[1] = "NonCensoredProbability";
            outputTable.ColumnNames[2] = "CensoredProbability";
        ...
        public object GetDataObject(Table inputTable, int row)
        {
            ...
            {
                horTh = inputTable[row, "horTh"],
                age = inputTable[row, "age"],
                menostat = inputTable[row, "menostat"],
                tsize = inputTable[row, "tsize"],
                tgrade = inputTable[row, "tgrade"],
                pnodes = inputTable[row, "pnodes"],
                progrec = inputTable[row, "progrec"],
                estrec = inputTable[row, "estrec"],
                time = inputTable[row, "time"],
                cens = inputTable[row, "cens"]
            };
            return breastCancer;
        }
