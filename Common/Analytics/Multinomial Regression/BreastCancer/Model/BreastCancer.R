...
# Applying Multinomial Regression Model to predict Adjusted
breastCancerFormula = formula(cens ~ horTh + age + menostat + tsize + tgrade + pnodes + progrec + estrec + time)
breastCancer_Multinom = multinom(breastCancerFormula ,trainData, trace=F)
summary(breastCancer_Multinom)
...
breastCancerTestProbabilities = predict(breastCancer_Multinom, type = "probs",testData)
...
CrossTable(breastCancerTestPrediction,testData$cens,  prop.chisq = FALSE, prop.t = FALSE, prop.r = FALSE,
...
pred <- prediction(labels = testData$cens, predictions = breastCancerTestProbabilities)
...
# PMML generation
pmmlFile = pmml(breastCancer_Multinom,data=trainData)
...
breastCancerEntireProbabilities = predict(breastCancer_Multinom, type = "probs",breastCancerOriginal)
