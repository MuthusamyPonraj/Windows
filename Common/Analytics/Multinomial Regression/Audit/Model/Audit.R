...
audit_Multinom<- multinom(auditFormula,trainData, trace=F)
summary(audit_Multinom)
...
auditTestProbabilities = predict(audit_Multinom, type = "probs",testData)
...
# PMML generation
pmmlFile = pmml(audit_Multinom , data=trainData)
...
auditEntireProbabilities = predict(audit_Multinom, type = "probs",auditOriginal)
