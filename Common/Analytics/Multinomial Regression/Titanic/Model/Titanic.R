...
# Frame the data. 
Survived = as.character(as.integer(titanic$survived == 'survived'))
titanic = cbind(titanic[, 1:6], Survived)
...
# Applying Multinomial Regression Model to predict Survived.
titanicFormula = formula(Survived~sex+age+sibsp+parch)
titanic_Multinom = multinom(titanicFormula ,trainData, trace=F)
summary(titanic_Multinom)
...
titanicTestProbabilities = predict(titanic_Multinom, type = "probs",testData)
...
# PMML generation
pmmlFile<-pmml(titanic_Multinom,data=trainData)
...
titanicOriginal = cbind(titanicOriginal[, 1:6], Survived)

# Applying Multinomial Regression model to entire dataset and save the results in a CSV file
titanicEntireProbabilities = predict(titanic_Multinom, type = "probs",titanicOriginal)
